...
class EBMAOAssignmentProposal:
    ...
    def propose(self, state):
        ...
        mode="guided":
            EBMAO-aware guided proposals based on distance,
    ...
    def _random_swap(self, state):
        """
        Randomly reassign exactly one task.
    ...
    def _select_tasks(self, state, k):
        """
        Select tasks according to the EBMAO assignment structure:

            ||s_a - c_i||²
            - lambda_align  * s_a^T c_i
            - lambda_memory * s_a^T kappa_a

        Larger values indicate tasks whose current assignment
        is comparatively less attractive.
        """
